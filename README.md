...
```shell
> cue cmd patch github.com/deepbrook/argocd-cmp-cuelang/config@v0.0.16 > patch.yaml
```

If you'd like to use a separate configMap only requires `-t cm=true`:

```shell
> cue cmd patch github.com/deepbrook/argocd-cmp-cuelang/config@v0.0.16 -t cm=true > patch.yaml
```
